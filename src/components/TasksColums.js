import OneTask from "./secondary-components/OneTask";
import { useState, useEffect } from "react";
import NewTask from "./secondary-components/NewTask";

const TasksColumns = (props) => {
  ...
  const renderMore = () => {
    if (more === "clicked") {
      return (
        <NewTask
          actualStatus={p}
          handleAddToData={props.handleAddToData}
  ...
  return (
    <>
      ...
      <main
        className={`${
          window.matchMedia("(min-width: 768px)").matches ? "hstack" : "vstack "
        } gap-3 container`}
      >
